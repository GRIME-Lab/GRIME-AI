...
#
# Companion to ROI_Analyzer. Shares its folder-pairing convention
...
class _SandbarEdgeSettings:
    ...
    so ProcessImage.canny_edges() drives off the Sandbar tab's spin
...
class Sandbar_Analyzer:
    ...
    # ------------------------------------------------------------------
    # folder pairing (mirrors ROI_Analyzer.generate_file_pairs)
    # ------------------------------------------------------------------
    @staticmethod
    ...
    def _compute_edges(self, operator, gray, canny_low, canny_high,
        ...
        app's single edge engine, ProcessImage, so a Canny/Sobel/
